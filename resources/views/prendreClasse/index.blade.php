...
<div class="layout-page">
    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <a class="btn btn-primary" href="{{ route('prendreClasse.saveForm') }}"><span class="icon-style">+</span> Ajouter {{ $title }}</a>
            <div class="card mt-3">
                <main class="cd__main">
                    <table id="example" class="table table-striped" style="width:100%">
                        ...
                        <tbody>
                            @foreach ($prendreClasse as $item)
                                <tr>
                                    <td>{{ $item->nom }} {{ $item->prenoms }}</td>
                                    <td>{{ $item->matiere->nom }}</td>
                                    <td>
                                        @php
                                            $classes = $item->classes->map(fn ($classe) => $classe->nom);
                                    ...
                                    <td>
                                        ...
                                        {{-- <a class="btn btn-primary" href="{{ route('prendreClasse.updateForm', [$item->id]) }}" disabled><i class="bx bx-edit-alt me-1"></i> Modifier</a> --}}
                                        <a class="btn btn-warning" href="{{ route('prendreClasse.delete', [$item->id]) }}"><i class="bx bx-trash me-1"></i> Suprimer</a>
                                    </td>
                                </tr>
                            @endforeach
                        <tbody>
        ...
        <div class="mt-3">
            ...
            <div class="modal fade" id="saveModal" data-bs-backdrop="static" tabindex="-1" style="display: none;" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <form class="modal-content" method="POST" action="{{ route('prendreClasse.save') }}" id="saveForm">
                        @csrf
