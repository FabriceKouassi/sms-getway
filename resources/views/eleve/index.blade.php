...
<div class="layout-page">
    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <a class="btn btn-primary" href="{{ route('eleve.saveForm') }}"><span class="icon-style">+</span> Ajouter {{ $title }}</a>
            <div class="card mt-3">
                <main class="cd__main">
                        ...
                        <tbody>
                            @foreach ($eleve as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->contact }}</td>
                                    <td>{{ $item->_parent->nom }} {{ $item->_parent->prenoms }}</td>
                                    <td>
                                        {{-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#saveModal">
                                            Modifier
                                        </button> --}}
                                        <a class="btn btn-primary" href="{{ route('eleve.updateForm', [$item->id]) }}"><i class="bx bx-edit-alt me-1"></i> Modifier</a>
                                        <a class="btn btn-warning" href="{{ route('eleve.delete', [$item->id]) }}"><i class="bx bx-trash me-1"></i> Suprimer</a>
                                    </td>
                                </tr>
                            @endforeach
                        <tbody>
        ...
        <div class="mt-3">
            ...
            <div class="modal fade" id="saveModal" data-bs-backdrop="static" tabindex="-1" style="display: none;" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <form class="modal-content" method="POST" action="{{ route('eleve.save') }}" id="saveForm">
                        @csrf
